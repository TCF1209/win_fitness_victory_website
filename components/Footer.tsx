...
export default function Footer() {
  return (
    <footer className="relative bg-navy">
      ...
      {/* Decorative slashes — left side */}
      <div className="absolute top-8 left-6 hidden md:block pointer-events-none">
        <svg width="20" height="60" viewBox="0 0 20 60" fill="none" className="opacity-20">
          <line x1="15" y1="0" x2="3" y2="55" stroke="#00c2ff" strokeWidth="1.5" />
          <line x1="20" y1="3" x2="8" y2="58" stroke="#00c2ff" strokeWidth="1" />
        </svg>
      </div>
          ...
          <div className="flex flex-wrap justify-center items-center gap-4 md:gap-6">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="text-muted hover:text-white/80 transition-colors text-sm uppercase tracking-wider"
              >
                {link.label}
              </a>
            ))}
